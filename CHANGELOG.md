...
sessions (not runs) are the unit of analysis, quantiles use Harrell-Davis with
n-matching, new scenarios get a fitted familiarisation curve instead of a
biased early baseline, benchmark progress becomes the primary headline with a
matched-vs-all-cells split, a hypothesis-generation "what's moving it?" panel
tests candidate causes against a negative-control distribution, and PB finally
gets held to the standard it's owed: `pb_surprise`, using an exact
expected-max table instead of the usual (and, at this app's sample sizes,
wrong) approximation.**

### The unit of analysis is the session

- Runs inside a session share warm-up state, fatigue, hardware and time of
  day — they are not independent draws. Every downstream statistic now
  reduces each session to one `session_value` first; no standard error or
  sufficiency gate is computed from a raw run count anymore.
- Reset detection switched from the old "zero elapsed seconds" heuristic
  (which over-fired ~30x) to `Avg FPS` not finalising, and warm-up exclusion
  is now indexed on elapsed time instead of run position.

### Harrell-Davis quantiles, n-matched

- Ceiling/Typical/Floor now use the Harrell-Davis quantile estimator (with
  its own standard error) instead of a plain order-statistic percentile, and
  comparisons across windows are n-matched so a longer window can't win
  purely by sampling deeper into its own tail.

### New scenarios: a fitted familiarisation curve, not a biased baseline

- A cell's first runs are the most familiarisation-contaminated it will ever
  produce. Early performance is now read off a fitted exponential
  familiarisation curve (`score(k) = A - B * exp(-(k+E)/lambda)`) instead of
  averaging those first runs directly, and a cell stays flagged "in
  familiarisation" (excluded from the matched basket) until it clears roughly
  3 lambda's worth of runs.

### Benchmark progress is the new primary headline

- Aggregating a fixed benchmark basket sidesteps the index-number problem
  that a free-scenario mix has, and its rank thresholds are anchored outside
  your own variance — so it's the one metric in the app with a genuinely
  tractable sample size. It now leads; the free-scenario aggregate is
  retained underneath as the secondary, weaker figure.
- Every cross-cell aggregate (benchmark and free) now reports **two**
  headline numbers side by side: **Matched** (cells with real data on both
  sides — the defensible skill-change figure) and **All cells** (adds
  fitted-asymptote fallbacks for brand-new scenarios, clearly flagged). Where
  they diverge, that gap *is* the familiarisation signal, surfaced rather
  than hidden.

### "What's moving it?" — hypothesis generation, not proof

- A new panel per benchmark ranks candidate scenarios by a dose-response test
  (weekly play volume vs. weekly outcome, Spearman/Pearson + OLS slope)
  against a same-sized negative-control distribution — scenarios picked
  without regard to plausibility. Clearing the null is a percentile rank
  worth testing further, explicitly not a p-value and not causal proof; the
  panel says so, and names its own `n_comparisons`.

### PB gets its `pb_surprise`, and an exact table to back it

- Added an exact table of `E[max of n draws from N(0,1)]` for n = 1..300,
  generated by Simpson's-rule numerical integration rather than Blom's
  approximation — which the measurement spec documents as unreliable exactly
  in this app's own 15-40 run session range, and only trustworthy past
  n ≈ 100. Beyond n = 300 it falls back to the asymptotic formula, anchored
  to the table's own boundary value so it can never regress.
- Every scenario card's PB row now shows `pb_surprise`: how many standard
  deviations the record sits from the maximum an unchanging player would be
  expected to reach from that many runs alone — gated below n = 10, same
  floor as CV. Near zero means the record is exactly what chance predicts
  from that much play; it is still never turned into a percentage.

### Fragmentation readout

- Below the fold on why so much reads "within noise": a plain accounting of
  how many cm/sensitivity cells exist against how many currently have enough
  sessions on both sides of the window to measure a change at all.
